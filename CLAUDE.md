...
TransBot은 환경 변수 기반 설정 관리 시스템을 사용합니다. 모든 설정은 `.env` 파일 또는 환경 변수로 관리되며, 설정되지 않은 값은 기본값이 사용됩니다.
...
Azure 사용 시 주의할 에러 상황:

1. **필수 파라미터 누락**

   ```python
   # app.py의 setup_api_client()에서 검증
   if provider == "azure":
