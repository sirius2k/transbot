...
1-2시간 이내 완료 가능한 간단한 개선사항은 MINOR-XXX ID로 관리됩니다.

| ID | 개선사항 | 우선순위 | 소요 시간 | 설명 | 이슈 |
| ---- | -------- | -------- | --------- | ---- | ---- |
| ✅ MINOR-001 | 지우기 버튼 | **P1** | 30분 | 입력 및 번역 결과 초기화, 사용성 개선 | - |
| ✅ MINOR-002 | 콘솔에서의 로깅 간소화 | **P2** | 10분 | 콘솔과 파일에 서로 다른 포맷터 적용. 콘솔은 사람이 읽기 쉬운 형식, 파일은 JSON 유지 | - |
| ✅ MINOR-003 | UI 레이아웃 개선 - 안내 문구 | **P2** | 30분 미만 | 안내 문구를 사이드바의 Help 섹션으로 이동. 메인 화면이 더 깔끔하게 정리됨 | - |
| ✅ MINOR-004 | Langfuse Prompt 표시 개선 | **P2** | 30분 | Langfuse Generation에 Prompt 섹션 추가. LLM 호출 시 시스템/사용자 메시지 확인 가능 | - |
| ✅ MINOR-005 | 모델 선택 옵션 제한 | **P1** | 1-2시간 | 환경 변수에 설정된 모델만 UI에 표시. API 키 미설정 모델 선택 방지. 사용자 혼란 최소화 | - |
...
| ID | 개선사항 | 우선순위 | 소요 시간 | 설명 | 이슈 |
| ---- | -------- | -------- | --------- | ---- | ---- |
| 🔲 MINOR-006 | Langfuse 메타데이터 확장 | **P1** | 1-2시간 | Langfuse Trace/Generation에 설정 메타데이터 추가 (번역 스타일, 추가 옵션, 모델 설정, UI 상태). 프롬프트 비교 분석, 사용 패턴 파악, 디버깅 개선, 품질 모니터링 | - |
| 🔲 MINOR-007 | 번역 결과 포맷팅 | **P2** | 1시간 | 번역 결과를 자동으로 포맷팅하여 출력 (줄바꿈, 들여쓰기 보존). 포맷 제거 옵션 제공 | [#57](https://github.com/sirius2k/transbot/issues/57) |
...
- **단위 테스트**: pytest 기반
- **통합 테스트**: pytest-bdd + Playwright 기반 브라우저 자동화 테스트
...
| ------------- | ------------------------ | ---------------------- |
| 코드 커버리지 | 80% 이상                 | ✅ **98%** 달성        |
| 단위 테스트   | 모든 핵심 함수 및 클래스 | ✅ **201개** 완료      |
| 통합 테스트   | 주요 사용자 시나리오     | ✅ **20개** 완료 (6개 TC) |

**단위 테스트 대상**: utils.py, components/ (language, text, translation, style_translator, observability), config.py, app.py

**통합 테스트 케이스** (docs/test-cases/):

| TC 번호 | 테스트 케이스 | 우선순위 | 시나리오 수 | 상태 |
| ------- | ------------- | -------- | ----------- | ---- |
| TC-001  | 페이지 렌더링 (Smoke Test) | P0 | 4 | ✅ 완료 |
| TC-002  | 번역 E2E 플로우 | P0 | 4 | ✅ 완료 |
| TC-003  | 텍스트 분석 및 통계 | P1 | 4 | ✅ 완료 |
| TC-004  | UI 인터랙션 | P1 | 5 | ✅ 완료 |
| TC-005  | 설정 관리 | P1 | 3 | ✅ 완료 |
| TC-006  | 에러 핸들링 | P1 | 3 | 🔲 작성중 |

**리포트**:

- 단위 테스트: htmlcov/ 폴더에서 커버리지 및 테스트 결과 확인 가능
- 통합 테스트: pytest-bdd HTML 리포트 생성 가능
