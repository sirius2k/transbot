...
```text
Phase 2 (P1 기능)
├── 1. 환경 변수 기반 설정 관리     ← Azure OpenAI 지원의 전제조건, 최우선
├── 2. Azure OpenAI Service 지원   ← 설정 관리 완료 후 구현 가능
├── 3. 번역 히스토리 저장 및 관리  ← 다른 기능의 기반, 독립적 개발 가능
└── 4. 사전 검색 기능              ← 독립적, 병렬 개발 가능
...
Phase 3 (P2-P3 기능)
├── 7. 외부 툴 연동 (Notion 우선)  ← 가장 범용적인 것부터
├── 8. 용어집(Glossary) 기능
└── 9. Langfuse 연동              ← 운영 안정화 후
```
