...
- 사용자가 최대 길이를 초과하는 매우 긴 텍스트를 입력 (10,000자 이상)
...
- 사용자가 최대 길이의 80% 이상인 긴 텍스트를 입력 (8,000자)
...
| 항목 | 값 | 설명 |
| ---- | -- | ---- |
| 긴 텍스트 (초과) | "A" * 10001 | 최대 길이 초과 (10,001자) |
| 긴 텍스트 (경고) | "B" * 8000 | 최대 길이 80% (8,000자) |
...
- **입력 길이 검증**: Config.MAX_INPUT_LENGTH (기본 10,000자) 기준
- **동적 메시지**: 에러 메시지 내용이 입력 길이에 따라 동적으로 생성됨
- **페이지 리렌더링**: 입력이 변경될 때마다 페이지가 리렌더링되므로 대기 시간 필요
- **80% 경고**: 8,000자 이상 입력 시 경고 메시지 표시 (Config.MAX_INPUT_LENGTH * 0.8)
