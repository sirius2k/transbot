...
**완료 조건**:
- [ ] STYLE_INSTRUCTIONS_EN_TO_KO 상수가 5가지 스타일 포함
- [ ] translate_single_style()이 영→한 번역 지원
- [ ] auto_select_styles_for_short_text() 메서드 제거 완료
...
**완료 조건**:
- [ ] 앱 시작 시부터 스타일 옵션이 표시됨
- [ ] 영→한 번역에서도 스타일 옵션 사용 가능
- [ ] 직역이 체크박스 제일 위에 표시됨
- [ ] 직역이 기본 선택됨
- [ ] 짧은 텍스트에도 동일한 UI 표시
...
**완료 조건**:
- [ ] README.md에 변경사항 반영
- [ ] PRD.md에서 FEATURE-024 완료 표시
- [ ] 모든 문서에서 markdownlint 규칙 준수
...
|------|-----|-------|-------|------|
| 24.1 | 🔲 예정 | 0% | - | - |
| 24.2 | 🔲 예정 | 0% | - | Task 24.1 완료 후 시작 |
| 24.3 | 🔲 예정 | 0% | - | Task 24.1, 24.2 완료 후 시작 |
| 24.4 | 🔲 예정 | 0% | - | 모든 구현 완료 후 시작 |

## 완료 기준

### 기능 완료 기준

- [ ] 앱 시작 시부터 스타일 옵션이 사이드바에 표시됨
- [ ] 영→한 번역에서도 5가지 스타일 옵션 사용 가능
- [ ] 한→영 번역에서도 기존처럼 5가지 스타일 옵션 사용 가능
- [ ] "직역" 스타일이 체크박스 목록 제일 위에 표시됨
- [ ] "직역" 스타일이 기본으로 선택됨
- [ ] 짧은 텍스트에도 동일한 스타일 선택 UI 표시
- [ ] AI 자동 스타일 선택 로직이 제거됨
- [ ] 사용자가 선택한 스타일로만 번역이 수행됨

### 품질 기준

- [ ] 모든 기존 테스트 통과
- [ ] 새로운 테스트(영→한 스타일) 추가 및 통과
- [ ] 코드 커버리지 80% 이상 유지
- [ ] README.md 및 관련 문서 업데이트 완료
- [ ] Markdownlint 규칙 준수
